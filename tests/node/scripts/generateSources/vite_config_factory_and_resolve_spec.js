...
test("vite.config.js factory (build): aliases, chunk groups, plugin order", async () => {
  ...
  assert.equal(cfg.plugins[1].name, "bundled-css-after-bulma");
  if (process.platform === "win32") {
    assert.equal(cfg.plugins[2].name, "copy-spritesheets-robocopy");
  } else {
    assert.equal(cfg.plugins[2].name, "vite:plugin:run");
  }
});
...
test("vite.config.js factory (serve): metadata aliases and chunk groups match build", async () => {
  ...
  assert.equal(cfg.plugins[0].name, "vite-plugin-item-metadata");
  assert.equal(cfg.plugins[1].name, "bundled-css-after-bulma");
  assert.equal(cfg.plugins[2].name, "dynamic assets");
});
